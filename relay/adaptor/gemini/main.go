...
func ConvertRequest(textRequest model.GeneralOpenAIRequest) *ChatRequest {
	model.SanitizeGeminiCompatibleRequestV2(&textRequest)
...
func (g *ChatResponse) GetResponseText() string {
	return geminiResponseTextOnlyV2(g)
}
	...
	for i, candidate := range response.Candidates {
		...
		if len(candidate.Content.Parts) > 0 {
			if candidate.Content.Parts[0].FunctionCall != nil {
				...
				choice.Message.Content = geminiCandidateContentV2(&candidate)
			}
		} else {
...
func streamResponseGeminiChat2OpenAI(geminiResponse *ChatResponse) *openai.ChatCompletionsStreamResponse {
	...
	choice.Delta.Content = geminiResponseContentV2(geminiResponse)
...
func StreamHandler(c *gin.Context, resp *http.Response) (*model.ErrorWithStatusCode, string) {
	...
	for scanner.Scan() {
		...
		responseText += geminiResponseTextOnlyV2(&geminiResponse)
...
func Handler(c *gin.Context, resp *http.Response, promptTokens int, modelName string) (*model.ErrorWithStatusCode, *model.Usage) {
	...
	completionTokens := openai.CountTokenText(geminiResponseTextOnlyV2(&geminiResponse), modelName)
	usage := model.Usage{
