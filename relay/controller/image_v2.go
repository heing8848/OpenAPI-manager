...
func RelayImageHelperV2(c *gin.Context, relayMode int) *relaymodel.ErrorWithStatusCode {
	...
	logger.Infof(
		c.Request.Context(),
		"processing standalone image generation request via channel_type=%s channel_id=%d model=%s",
		getStandaloneOutputChannelNameV2(metaInfo.ChannelType),
		metaInfo.ChannelId,
		getStandaloneOutputModelNameV2(metaInfo),
	)
	return RelayImageHelper(c, relayMode)
}
