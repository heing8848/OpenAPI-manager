...
func RelayAudioHelper(c *gin.Context, relayMode int) *relaymodel.ErrorWithStatusCode {
	...
	if relayMode == relaymode.AudioSpeech {
		if speechErr := RelayAudioSpeechResponseHandlerV2(c, resp, meta); speechErr != nil {
			return speechErr
		}
	...
	defer func(ctx context.Context) {
		go billing.PostConsumeQuota(ctx, tokenId, quotaDelta, quota, userId, channelId, channelKeyId, channelKeyIndex, modelRatio, groupRatio, audioModel, tokenName)
	}(c.Request.Context())
