...
func (channel *Channel) addAbilitiesTx(tx *gorm.DB) error {
	models_ := splitChannelModelsV2(channel.Models)
...
func (channel *Channel) UpdateAbilities() error {
	return channel.updateAbilitiesTx(DB)
}

func (channel *Channel) updateAbilitiesTx(tx *gorm.DB) error {
	// A quick and dirty way to update abilities
	// First delete all abilities of this channel
	err := channel.deleteAbilitiesTx(tx)
	if err != nil {
		return err
	}
	// Then add new abilities
	err = channel.addAbilitiesTx(tx)
	if err != nil {
		return err
	}
	return nil
