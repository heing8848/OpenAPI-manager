...
func loadOptionsFromDatabase() {
	...
	for _, option := range options {
		...
		if option.Key == "Theme" {
			normalizedTheme := config.NormalizeTheme(option.Value)
			if normalizedTheme != option.Value {
				option.Value = normalizedTheme
				DB.Model(&Option{}).Where("key = ?", option.Key).Update("value", normalizedTheme)
			}
		}
...
func updateOptionMap(key string, value string) (err error) {
	...
	case "Theme":
		...
		config.OptionMap[key] = config.Theme
	}
	return err
}
