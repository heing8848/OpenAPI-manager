...
func migrateDB() error {
	...
	if err = MigrateChannelKeys(); err != nil {
		return err
	}
	return nil
}
...
func setDBConns(db *gorm.DB) *sql.DB {
	...
	if config.LowMemoryMode {
		...
		defaultConnMaxLifetime = 180
	}
	if common.UsingSQLite {
		// SQLite 在低配机器上更容易被过大的连接池拖垮，保守一点更稳。
		defaultMaxIdleConns = 1
		defaultMaxOpenConns = 1
		defaultConnMaxLifetime = 0
	}

	sqlDB.SetMaxIdleConns(env.Int("SQL_MAX_IDLE_CONNS", defaultMaxIdleConns))
	sqlDB.SetMaxOpenConns(env.Int("SQL_MAX_OPEN_CONNS", defaultMaxOpenConns))
	sqlDB.SetConnMaxLifetime(time.Second * time.Duration(env.Int("SQL_MAX_LIFETIME", defaultConnMaxLifetime)))
	return sqlDB
}
