...
const EditChannel = (props) => {
    ...
    const submit = async () => {
        if (!isEdit && (inputs.name === '' || (inputs.key.trim() === '' && (inputs.base_url || '').trim() === ''))) {
            showInfo('请填写渠道名称，并至少提供一种接入信息（API 密钥或 Base URL）！');
            return;
        }
        if (inputs.type === 52 && (inputs.base_url || '').trim() === '') {
            showInfo('Video Task (V1) 必须填写 Base URL。');
            return;
        }
                ...
                <Spin spinning={loading}>
                        ...
                        <>
                            <div style={{ marginTop: 10 }}>
                                <Banner type={"warning"} description={
                                    <>
                                        注意，<strong>模型部署名称必须和模型名称保持一致</strong>，因为 Alfred API 会把请求体中的
                                        model
                    ...
                    }
                    {
                      inputs.type === 52 && (
                        <>
                            <div style={{ marginTop: 10 }}>
                                <Typography.Text strong>Video Task Base URL：</Typography.Text>
                            </div>
                            <Input
                              name='base_url'
                              placeholder={'请输入任务型视频上游的 Base URL'}
                              onChange={value => {
                                  handleInputChange('base_url', value)
                              }}
                              value={inputs.base_url}
                            ...
                            <div style={{ marginTop: 10 }}>
                                <Banner
                                  type={'info'}
                                  description={'Video Task (V1) 仅支持手动填写模型，V1 暂不支持自动拉取模型列表。'}
                                />
                            </div>
                        </>
                      )
