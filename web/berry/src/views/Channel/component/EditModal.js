...
const validationSchema = Yup.object().shape({
  ...
  key: Yup.string().test(
    ...
    '创建渠道时请至少填写 API 密钥或 Base URL',
    function (value) {
      const { is_edit, base_url, type, config } = this.parent;
      if (is_edit) {
        return true;
      }
      ...
      if (type === 33 && config?.ak && config?.sk && config?.region) {
        return true;
      }
      if (type === 42 && config?.region && config?.vertex_ai_project_id && config?.vertex_ai_adc) {
        return true;
      }
      return false;
    }
