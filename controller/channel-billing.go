...
func updateChannelCloseAIBalance(channel *model.Channel) (float64, error) {
	apiKey := channel.ResolvePrimaryKeyValue()
	if apiKey == "" {
		return 0, errors.New("channel has no available key")
	}
	url := fmt.Sprintf("%s/dashboard/billing/credit_grants", channel.GetBaseURL())
	body, err := GetResponseBody("GET", url, channel, GetAuthHeader(apiKey))
...
func updateChannelOpenAISBBalance(channel *model.Channel) (float64, error) {
	...
	url := fmt.Sprintf("https://api.openai-sb.com/sb-api/user/status?api_key=%s", apiKey)
	body, err := GetResponseBody("GET", url, channel, GetAuthHeader(apiKey))
	if err != nil {
		return 0, err
	}
